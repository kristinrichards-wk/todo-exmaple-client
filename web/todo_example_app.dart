...
main() async {
  ...
  if ((await sessionApi.getAuthenticationStatus()).loggedIn) {
    ...
    HttpProvider clientProvider = new HttpProvider()..use(oauth2.httpInterceptor);
    Options options =
    new Options('http://localhost:8100', membershipId, clientProvider);
    Client client = new Client(options);
    await client.open();

    TodoService todoService = new WdeskTodoService(client, shell.session, oauth2);
    ...
    var component = shell.components.content(
        content: mainContent, menuContent: todoModule.components.sidebar(), menuHeader: null);
